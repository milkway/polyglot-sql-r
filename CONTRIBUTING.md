...
The Rust sources live in `src/rust/`. After changing exported `#[extendr]`
...
2. run the test suite (`devtools::test()`), which asserts the linked crate
   version matches `DESCRIPTION`;
